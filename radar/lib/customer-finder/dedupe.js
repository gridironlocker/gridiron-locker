...
export function fingerprint({ platform, url, author, text }) {
  const basis = [String(platform || "").toLowerCase(), canonicalUrl(url), String(author || "").toLowerCase().trim(), normalizeText(text)].join("|");
