...
Internal ops dashboard: `ops/scout/` is generated by `src/scout.py` on every build — FTI
leaderboard, catalogue health, design gaps, headlines and dead-campaign tracking. It is
**repo-local by design**: `ops/` and `marketing/` are never copied into `site/`, because GitHub
Pages serves every file in the artifact and `noindex` plus a robots.txt `Disallow` is not access
control. Open them from the repo instead:

```bash
python3 -m http.server 8000 --bind 127.0.0.1   # then /ops/scout/ and /marketing/dashboard.html
```
...
- **robots.txt rewritten**: welcomes Google, Bing, Google Images and AI answer engines
  ...
  crawler while looking sitewide. There are **no `Disallow` lines at all**: the internal rooms
  (`/marketing/`, `/ops/`) are no longer deployed, and a `Disallow` in a public robots.txt only
  advertises a path without protecting it.
