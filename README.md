...
| Google Shopping feed | **84 items** at `products.xml` - rebuilt with the site (see [Google Shopping](#google-shopping-merchant-center) below) |
| Product images | self-hosted (Viralstyle + Mayzing localised by `dl.py`; CDN hot-links are fallback only) |
...
The **Shopping tab** is the one surface on Google Search that organic SEO can't reach. It is
fed *only* by approved Google Merchant Center (GMC) product feeds — GA4, Search Console and the
on-page Product schema do not put products there, they only help pages rank and earn rich
results. So the build emits one feed at a stable URL on every refresh:

**`https://gridironlocker.store/products.xml`** — Content API for Shopping format, one `<item>`
per live design, regenerated by `src/build.py::shopping_feed()`. It carries the same catalogue,
prices, images and partner-scoped shipping terms as the product pages (Viralstyle items declare
the published $4.95 US flat rate; Mayzing items declare none, because Mayzing publishes no
rate). `tests/test_layout.py::GoogleShoppingFeed` fails the suite if the feed ever drifts from
the merged catalogue.

### One-time setup (~20 minutes, in your Google account)

1. **Create the account:** merchant.google.com → Start now, with the Google account that owns
   the site's Search Console property. Business name *Gridiron Locker*, website
   `https://gridironlocker.store`, country **United States**, currency **USD**.
2. **Verify the domain:** Business info → Verify domain. It is already verified in Google
   Search Console, so the Search Console option should confirm it in one click (fall back to
   the HTML-file option — `googleae06215486ed6c17.html` is already served — if not).
3. **Add the feed:** Products → Items → *Add products* → *Manually upload products* →
   **XML file** → paste `https://gridironlocker.store/products.xml`. Set the refresh frequency
   to **every 24 hours** (the feed is actually rebuilt twice a day by the refresh workflow).
4. **Shipping:** Business info → Shipping → add a US delivery option (flat rate is fine; it is
   the fallback for the Mayzing items, while Viralstyle items carry the per-item rate in the
   feed).
5. **Returns (recommended):** Business info → Returns → a US 30-day policy (the Viralstyle
   product pages publish exactly that window — set Mayzing to what you can actually honour).
6. **Wait for the first import** — usually a few hours, up to a day or two. Items move
   *Pending → Active* under Products → All. Read anything it reports under *Diagnostics*:
   the feed is built from verified page data, so a diagnostic there is worth answering.
7. **Recommended:** link a Google Ads account (Business info → Linked accounts). Free listings
   appear in the Shopping tab without ads, but the link lets you bid on Shopping campaigns
   later without redoing any of this.

### What to expect

- The Shopping tab shows these **free listings** on product queries (e.g. "packers t shirt")
  for US users — the feed is `content_country=US`. The tab does not appear on every query, and
  products start appearing **days** after the feed is first imported, not minutes.
- Custom labels 0–2 carry collection / partner / garment, so future Shopping campaigns can
  segment by team and fulfilment partner.
- **Honest caveat:** the transaction finishes on the fulfilment partner's store
  (viralstyle.com / gridironlocker.shop), not on this domain. Google's merchant policies
  prefer the purchase to happen on the merchant's own website, so watch
  *Diagnostics → Policy* during the first week for any website-related flag. The product pages
  are complete, priced and identical to the feed — that landing-page match is the first thing
  Google checks, and it is satisfied.
