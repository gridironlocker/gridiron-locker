...
Internal ops dashboards (`ops/scout/`, `ops/hq/`, `ops/board/`, `ops/seo/`, `marketing/dashboard.html`)
are generated on every build but are **withheld from the public site** since 2026-09-20 - noindex
plus a robots Disallow is not access control, and the commercial data they carry does not belong
on a public host. Preview them locally with `GL_PUBLISH_INTERNAL=1 python3 src/build.py` (they
are never deployed by the workflows, which run without that flag).
...
| Item | Count |
|---|---|
| HTML files in `site/` | **202 public** (owner dashboards are withheld from the artifact since 2026-09-20) |
| Public pages audited | **194** (`python3 qa_audit.py`) |
| Redirect stubs (retired slugs) | **85** |
| Product pages (one per design) | **69** (14 player-likeness designs delisted 2026-09-20 per DESIGN-BLUEPRINT §2) |
| `sitemap.xml` URLs | **93** = 69 products + 24 other indexable pages |
...
Fulfilment split: Viralstyle 37 / Mayzing 32 (after the 2026-09-20 IP delist).
...
> Product mockups are hot-linked from the Mayzing CDN (signed URLs, 38 unique images).
> Self-hosting them is the next resilience step - see SITE-AUDIT-2026-09-20.md.

### Retired campaigns (updated 2026-09-20)
`limited-edition-go-b-r-o-w-n-s`, `it-s-not-a-team-logo-browns-it-s-a-famil` and
`limited-edition-m` no longer return product data on Viralstyle. `limited-edition-grb41` is
**alive** (the earlier "dead" note was wrong) - it was delisted on 2026-09-20 under
DESIGN-BLUEPRINT §2 (player likeness) together with 13 other player designs. Slugs listed in
`data/delisted.json` can never be resurrected by a re-crawl; their URLs serve noindex 301 stubs.
...
- `site/feeds/pinterest.csv` - the Pinterest Catalogs data source (retail-catalogue spec:
  `id,title,description,link,image_link,price,availability,…`), one row per live design in
  `build.ALL`, regenerated by every build. Point Pinterest at
  `https://gridironlocker.store/feeds/pinterest.csv` (Catalogs → Add data source → URL). It is
  written straight into `site/` because `sync_marketing()` deliberately withholds every `.csv`
