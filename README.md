...
Internal marketing intelligence: **`/marketing/dashboard.html`** now includes an NFL Fan Commerce
Marketing Agent view. It is deliberately more than an SEO/content writer: it matches live headline
signals to existing products, scores fan psychology and apparel style, chooses platform-native
angles, builds game runways, exposes POD economics and funnel guardrails, proposes A/B tests, and
remembers measured learnings. It prepares drafts only; human approval and the IP/licensing gate are
mandatory. Run `python3 marketing/plan.py && python3 marketing/commercial_agent.py` to refresh it.
