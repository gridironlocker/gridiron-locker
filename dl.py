...
# Slugs the storefront will never publish, and therefore never show artwork
# for. Two lists, exactly the ones src/build.py:retired_slugs() uses to decide
# which /shop/<slug>/ URLs become text-only redirect stubs:
#   * data/delisted.json      - retired designs (player/coach left, artwork pulled)
#   * data/fulfillment.json   - "hold": withheld from Viralstyle pending Mayzing
# Fetching their mockups used to be pure waste - build.py now prunes any file
# under site/img/ that no page references, so the download would be deleted
# again in the same refresh run. Skipping them here keeps the crawl from
# ping-ponging with the prune. A design that comes back to the catalogue
# reappears in the crawl and gets its artwork on the next pass.
def _unpublishable():
    out = set()
    try:
        with open('data/delisted.json', encoding='utf-8') as fh:
            out |= set(json.load(fh))
    except (OSError, ValueError):
        pass
    try:
        with open('data/fulfillment.json', encoding='utf-8') as fh:
            out |= set(json.load(fh).get('hold') or [])
    except (OSError, ValueError):
        pass
    return out
...
def main():
    ...
    held={k for k in _unpublishable() if k in P}
    if held:
        print('skipping',len(held),'retired/hold slugs (no published page -> no artwork needed):',
              ', '.join(sorted(held)[:6]) + (' ...' if len(held)>6 else ''))
    ...
    for slug,v in P.items():
        if slug in held:
            # No published page can ever show these, so no image is fetched and
            # no img map is written. build.py prunes any leftover file under
            # site/img/ that no page references, and that pass runs after this
            # one in the refresh workflow - fetching them would just be undone.
            continue
...
# AGENTS.md 6: this script used to crawl Viralstyle and rewrite site/img/ the
# moment anything imported it. The work now happens only when it is run.
