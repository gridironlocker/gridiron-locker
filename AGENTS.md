...
| `marketing/osint/**` | **Arena** (owner-directed) | Private prospect engine created 2026-09-17 at the owner's request; docs in `marketing/osint/README.md`. Its `state/` and `exports/` subdirs are gitignored and MUST stay that way — they hold prospect contact data. It never writes to `data/` or `site/`. Its only committed output is the aggregate-only `page/prospects.json` (counts + page-password hash, no PII — enforced by tests + `audit`), rendered by `src/prospects_page.py`. `osint-verify.yml` commits/deploys nothing and must not join `pages-deploy`. |
...
sanity gate), it carries every sellable slug with its collection, partner,
absolute storefront URL, buy URL, price and a `site_published` flag, and it
records how many slugs each gate excluded. **Read that file.** It is the only
sanctioned way for `marketing/` to know what is sellable, and
`tests/test_catalogue_contract.py` fails the build if it drifts from `build.ALL`.
...
- `data/catalogue-live.json` is written **only** by the storefront. Marketing
  reads it and never writes it — that is the one-directional exception to §4.2,
  and it exists so marketing never has to write to `data/` at all.
- To get the same list in Python without the file: `sys.path.insert(0, "src");
  import build; {i["slug"] for i in build.ALL}`. Importing `build` is read-only
  and has no side effects. `refresh.yml:114` already does this in its sanity gate.
...
`ops/prospects/` follows the same rule: generated by `src/prospects_page.py`
from the committed aggregate file `marketing/osint/page/prospects.json`
(`build-page` output), aggregate-only, noindex, robots-disallowed,
password-gated.
...
## 5. Known open issue — two competing planners (owner: ChatGPT, see CODEX-TASKS.md)

**This is the most serious defect in the repository and it is entirely inside
the marketing half.** Full detail and acceptance criteria are in
`CODEX-TASKS.md` Task 1. Summary, because it changes how you read `marketing/`:

There are **two divergent implementations of `build_plan()`** —
`marketing/plan.py:999` (187 lines, in a 1,203-line file, documented in
`marketing/README.md`'s "Separation contract") and `marketing/live_engine.py:199`
(124 lines, in a 361-line file, **not mentioned in any doc**).

`refresh.yml` runs `plan.py` and then `live_engine.py`, and `live_engine.py`
overwrites `marketing/plan.json` — its own comment says *"overwrites old
repetitive version"*. So the documented planner's output is computed and
discarded on every refresh, and everything downstream (`commercial_agent.py` →
`commercial-brief.json` → `three_day_pulse.py` → `publisher.py`) is fed by the
undocumented one.

Both read `data/products.json` — the **stale** 113-slug crawl — and neither
reads `data/fulfillment.json`'s `hold` list. Measured 2026-09-13:

- **55 of 96** queue rows point at designs with **no published page**, all
  carrying raw `viralstyle.com` URLs
- the **top 3** rows of `top_5_products_today` are three unpurchasable
  `sanders-*-special-edition` designs, outranking every real product
- **8 of 24** drops in `data/live_drops.json` point at unpublished designs

`live_engine.py:356` also writes `data/live_drops.json`, which contradicts §3.2,
§4.2 and `marketing/README.md`'s own contract — but it is load-bearing, since
`src/drops_page.py:36` reads it back. It needs a deliberate migration, not a
deletion.

The storefront is **safe** throughout: `src/build.py` filters drops by
`slug in lookup` and `src/drops_page.py:119` guards again. But it discards them
*silently*, so `/drops/` renders 16 of 24 and nothing reports the loss.

**Agreed interface, so neither agent edits the other's code:**

- **Arena — DELIVERED.** `src/catalogue_contract.py` emits
  `data/catalogue-live.json` from `build.ALL`, validating that every entry has a
  real published page, an absolute storefront URL, a buy URL and a known partner,
  and refusing to write a contract that disagrees with the build.
  `refresh.yml` runs it after the sanity gate and before the test suite, so the
  committed contract always matches the committed site.
  `tests/test_catalogue_contract.py` (15 tests) pins it: same slug set as
  `build.ALL`, matching counts, no unpublished designs, redirect stubs rejected,
  not stale, and strictly smaller than the raw crawl. Schema and field meanings
  are in `CODEX-TASKS.md` Task 1.
- **ChatGPT — OUTSTANDING.** Collapse to one planner, consume
  `catalogue-live.json` instead of the crawl, decide where `live_drops.json`
  should live, and make discarded drops visible. See `CODEX-TASKS.md` Task 1 for
  acceptance criteria.
